...
var renderElements = function (elements) {
  ...
  elements.forEach((element) => {
    var todo = `<div id="todo${element.todo_id}" class="flex-item col-xs-12 col-md-4">
          <div  data-search=${element.search_term} class="todo_item">
            <div class="search">${element.search_term}</div>
            <div class="category">${element.category}</div>
            <div id="todoContent${element.todo_id}" class="todo_content"></div>
          </div>
        </div>`;
    todoContainer.append(todo);
    contentElement(element.search_term, element.category, element.todo_id );
  });
...
var contentElement = function (title, category, id) {
  $.post('api/amazon', {title: title, category: category}, function (data) {
    $('#todoContent' + id)[0].innerHTML = data;
